...
export default async function processDtsFiles() {
  ...
  const mapFiles = await glob(`${tempTypes}/**/*.{d.ts.map,d.cts.map,d.mts.map}`);
  ...
  for (const filePath of dtsFiles) {
    ...
    const cjsFilePath = path.join(distCjs, relativeDir, `${baseFileName}${dtsExt.cjs}`);
    const esmFilePath = path.join(distEsm, relativeDir, `${baseFileName}${dtsExt.esm}`);

    // Copy .d.ts file to dist/esm/*.d.ts
    await fs.copy(filePath, esmFilePath);
    await normalizeDtsImports(esmFilePath, false);
    await updateSourceMappingURL(esmFilePath, `${baseFileName}${dtsMapExt.esm}`);

    // Copy .d.ts to dist/cjs/*.d.cts
    await fs.copy(filePath, cjsFilePath);
    await normalizeDtsImports(cjsFilePath, true);
    await updateSourceMappingURL(cjsFilePath, `${baseFileName}${dtsMapExt.cjs}`);
  }

  for (const mapFilePath of mapFiles) {
    const { relativeDir, baseFileName } = getCtx(mapFilePath);

    const cjsDtsFileMapPath = path.join(distCjs, relativeDir, `${baseFileName}${dtsMapExt.cjs}`);
    const esmDtsFileMapPath = path.join(distEsm, relativeDir, `${baseFileName}${dtsMapExt.esm}`);
